...
export const BodyComponent: React.FC<BodyComponentProps> = ({ onClick, onChange, partsInput, mode = {} }) => {
  ...
  const onClickSvg = ({ target }: any) => {
    ...
    if(mode === "missing")
    {
        ...
        if(id === "left_shoulder") ids = ["left_arm", "left_hand"];
        if(id === "left_arm") ids = ["left_hand"];

        if(id === "right_shoulder") ids = ["right_arm", "right_hand"];
        if(id === "right_arm") ids = ["right_hand"];

        if(id === "left_leg") ids = ["left_foot"];
        if(id === "right_leg") ids = ["right_foot"];


        ids.forEach((organ) => {
            if (newParts[organ] === true || newParts[organ] === false) {
                newParts[organ] = newParts[id]
            } else if (newParts[organ]) {
                newParts[organ].selected = newParts[id].selected
            } else {
                newParts[organ] = { selected: true }
            }
        } )
    }
    ...
    <Wrapper className="human-body">
      {Object.keys(svgElements)
        ...
        .map((part: string) => {
          ...
          return svgElements[part](selected, parts[part]?.clickable !== undefined ? parts[part].clickable : true)
        })}
    </Wrapper>
  )
}